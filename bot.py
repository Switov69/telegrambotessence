...
# --- КОНФИГУРАЦИЯ ---
BOT_TOKEN = "8407380994:AAGCg5GCEhVnMvurZg5Rrfuol0NxDffif2E"
...
media_group_locks: Dict[int, bool] = {} 

# Новое хранилище для сбора альбомов
...
async def release_media_lock(user_id: int, delay: float):
    await asyncio.sleep(delay)
    media_group_locks.pop(user_id, None)
...
async def handle_all_logic(message: Message):
    ...
    if message.media_group_id:
        ...
        if mg_id not in media_groups_storage:
            ...
            # Запускаем таймер на обработку всей группы через 0.6 сек
...
async def process_media_group(mg_id: str, user_id: int):
    ...
    await asyncio.sleep(0.7) # Оптимальное время ожидания всех частей
    messages = media_groups_storage.pop(mg_id, [])
    if not messages: return

    # Сортируем по ID сообщения для порядка
    messages.sort(key=lambda x: x.message_id)

    # Проверка на наличие текста (текст обычно в первом сообщении альбома)
    ...
    # Проверка лимитов (не более 2 фото)
...
async def send_to_moderation(user_id: int, messages: List[Message]):
    ...
    # Формируем медиа-группу для пересылки админам
    media_list = []
    caption = messages[0].caption or ""
    
    for i, m in enumerate(messages):
        current_cap = caption if i == 0 else None # Капшн только к первому элементу
        if m.photo:
            media_list.append(InputMediaPhoto(media=m.photo[-1].file_id, caption=current_cap, parse_mode="HTML"))
        elif m.video:
            media_list.append(InputMediaVideo(media=m.video.file_id, caption=current_cap, parse_mode="HTML"))
    ...
    for adm in admins:
        try:
            # Отправляем альбом или одиночный файл
            sent_msgs = await bot.send_media_group(chat_id=adm, media=media_list)
            # Привязываем кнопки к последнему сообщению из группы
            await bot.send_message(
                adm, 
                f"📨 Новый пост от {username}\nID: <code>{user_id}</code>", 
                reply_to_message_id=sent_msgs[-1].message_id,
                reply_markup=mod_kb,
                parse_mode="HTML"
            )
        except Exception as e:
            logger.error(f"Error sending to admin {adm}: {e}")
...
async def process_callbacks(callback: CallbackQuery):
    ...
    if data.startswith(("ap_", "re_")):
        ...
        if processed:
            ...
            await callback.answer(f"❗Этот пост уже {p_status} админом {p_admin}", show_alert=True)
            return

        # Получаем медиа из сообщения, на которое ответили кнопками
        source_msg = callback.message.reply_to_message
        if not source_msg:
            return await callback.answer("Ошибка: Сообщение удалено из кеша.", show_alert=True)

        if action == "ap":
            footer = f"\n\n<a href='{PEREXODNIK_LINK}'>Переходник</a> | <a href='{PREDLOZHKA_LINK}'>Предложка</a> | <a href='{CHAT_LINK}'>Чат</a>"
            
            try:
                # Если это был альбом (проверяем наличие media_group_id в исходнике админа)
                # Так как мы сами его переотправляли через send_media_group
                if source_msg.media_group_id:
                    # В aiogram 3.x нет простого способа получить всю группу по ID из одного сообщения без кеша
                    # Но так как мы работаем с 1-2 фото, мы можем просто опубликовать текущее + проверить наличие текста
                    # Но правильнее - взять caption и переслать само медиа
                    # Для надежности:
                    cap = (source_msg.caption or "") + footer
                    if source_msg.photo:
                        await bot.send_photo(CHANNEL_ID, source_msg.photo[-1].file_id, caption=cap, parse_mode="HTML")
                    elif source_msg.video:
                        await bot.send_video(CHANNEL_ID, source_msg.video.file_id, caption=cap, parse_mode="HTML")
                else:
                    # Одиночное
                    cap = (source_msg.caption or "") + footer
                    if source_msg.photo:
                ...
                await callback.message.edit_text(f"✅ <b>Одобрено</b> админом {admin_name}", parse_mode="HTML")
            ...
            await callback.message.edit_text(f"❌ <b>Отклонено</b> админом {admin_name}", parse_mode="HTML")
        ...
        try:
            ...
            await callback.message.edit_text(f"🗑 Пост <code>#{post_id}</code> успешно удален из канала админом {admin_name}.", parse_mode="HTML")
        except Exception as e:
            await callback.answer(f"Не удалось удалить: {e}", show_alert=True)
