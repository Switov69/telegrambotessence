    ...
    try:
        ...
        if os.path.exists(lock_file):
            ...
            with open(lock_file, 'r') as f:
                pid = f.read().strip()
            try:
                # Пытаемся завершить процесс (на Unix-системах)
                if os.name != 'nt':  # Не Windows
                    os.kill(int(pid), 0)
                print("⚠️ Обнаружен другой запущенный экземпляр бота")
                return True
            except:
...
async def handle_user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if is_main_admin(user_id) and update.message.text:
...
async def button_handler_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.error(f"Ошибка обработки админ-кнопки: {e}")
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Обрабатываем конфликт множественных экземпляров
        if isinstance(error, Conflict):
            logger.error("❌ ОШИБКА: Запущено несколько экземпляров бота! Остановите все копии и запустите заново.")
            print("❌ ОШИБКА: Обнаружено несколько запущенных копий бота!")
            print("💡 РЕШЕНИЕ: Нажмите Ctrl+C чтобы остановить бота, затем запустите заново")
            return
...
def main():
    ...
    if check_already_running():
        ...
        print("💡 РЕШЕНИЕ:")
        print("   1. Нажмите Ctrl+C в окне где запущен бот")
        print("   2. Закройте все окна командной строки")
        print("   3. Запустите бота заново")
        input("Нажмите Enter для выхода...")
        return
    
    # Регистрием очистку lock-файла при завершении
    import atexit
    ...
    try:
        ...
        # Создаем Application с настройками для лучшей стабильности
        ...
        print("   ✅ Защита от множественных запусков")
        print("")
